...
import { PenTool, Sparkles, Smile, LogOut, MessageSquare, Flame, Calendar, TrendingUp, Key, Mail, User, Send, Sun, Moon, ChevronRight, ChevronDown, FileText, RefreshCw, Cpu, BarChart3 } from 'lucide-react';
    ...
    <div style={{ minHeight: '100vh', width: '100vw', backgroundColor: colors.bg, color: colors.text, fontFamily: 'sans-serif', display: 'flex', transition: 'background-color 0.2s, color 0.2s', margin: 0, padding: 0, overflowX: 'hidden' }}>
      ...
      <main style={{ flex: 1, padding: '40px', overflowY: 'auto', maxHeight: '100vh', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', gap: '32px' }}>
        ...
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
          <div>
            <h2 style={{ margin: 0, fontSize: '28px', fontWeight: '800', letterSpacing: '-0.5px', color: colors.text }}>Journal Dashboard</h2>
            <p style={{ margin: '4px 0 0 0', color: colors.textMuted, fontSize: '14px' }}>Log your thoughts and navigate your logs dynamically by Month and Date.</p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            >
              <RefreshCw size={15} className={syncing ? 'animate-spin' : ''} style={{ transform: syncing ? 'rotate(360deg)' : 'none', transition: syncing ? 'transform 1s linear infinite' : 'none' }} />
        ...
        {/* PROMINENT AI CORE IDENTITY SECTION */}
        <section style={{ background: 'linear-gradient(135deg, #4f46e5, #6366f1)', padding: '28px', borderRadius: '24px', color: '#ffffff', display: 'flex', flexDirection: 'column', gap: '20px', boxShadow: '0 10px 25px -5px rgba(99, 102, 241, 0.4)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div>
                ...
                <p style={{ margin: '2px 0 0 0', opacity: 0.85, fontSize: '13px' }}>Unlocking metrics on psychological profiles, automated behavior cycles, and mood trajectory tracking vectors.</p>
              </div>
            </div>
            <span style={{ fontSize: '11px', fontWeight: '800', background: '#ffffff', color: '#4f46e5', padding: '6px 14px', borderRadius: '30px', letterSpacing: '0.5px' }}>ACTIVE ENGINE</span>
          </div>

          {/* INNER AI SUMMARY GRID */}
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '20px', marginTop: '4px' }}>
            ...
            <div style={{ background: 'rgba(255, 255, 255, 0.1)', borderRadius: '16px', padding: '20px', border: '1px solid rgba(255,255,255,0.15)' }}>
              ...
              {intervention && history.length > 0 ? (
                ...
                <p style={{ margin: 0, fontSize: '13.5px', opacity: 0.9 }}>Awaiting entry generation to run neural companion recommendations.</p>
              )}
            </div>

            {/* Live Forecasting Card */}
            <div style={{ background: 'rgba(255, 255, 255, 0.1)', borderRadius: '16px', padding: '20px', border: '1px solid rgba(255,255,255,0.15)', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              ...
              {history.length < 2 ? (
                <p style={{ margin: 0, fontSize: '13px', opacity: 0.85 }}>Requires 2 distinct logs to launch trends forecasting vectors.</p>
              ) : insights && (
                <div>
                  <div style={{ fontSize: '16px', fontWeight: '800', background: '#ffffff', color: '#10b981', display: 'inline-block', padding: '4px 12px', borderRadius: '8px', marginBottom: '6px' }}>
        ...
        {/* BOTTOM ROW: HABIT TRACKER FULL CONTAINER */}
        <div style={{ background: colors.card, border: `1px solid ${colors.border}`, padding: '32px', borderRadius: '24px', display: 'flex', flexDirection: 'column' }}>
                              ...
                              {isDateOpen && (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', marginTop: '10px', paddingLeft: '14px' }}>
                                  {logs.map((entry, idx) => {
                                    ...
                                    return (
                                      <div key={idx} style={{ padding: '16px', background: colors.inputBg, border: `1px solid ${colors.border}`, borderRadius: '12px' }}>
